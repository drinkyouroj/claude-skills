---
name: tcn-readability
description: >
  Audits a finished article draft for density and accessibility problems — paragraphs that run too
  ...
  the worst offenders. ALWAYS invoke this skill on a TCN article draft when the user says the
  writing feels "dense," "hard to read," "intimidating," "inaccessible," "too dense for non-experts,"
  ...
  could read it. Pairs with `tcn-text-humanizer` — this skill is the structural density audit;
  `tcn-text-humanizer` is the lexical AI-tell pass calibrated to Justin's voice (rewrites in place,
  not a report). Run `tcn-readability` first when both apply (structural fixes precede word-level
  cleanup). Does NOT apply to outlines, social media posts (use `tcn-post`), AI-tell hunting at the
  vocabulary level (use `tcn-text-humanizer`), fact-checking (use `tcn-fact-check`), or fixing weak
  openers specifically (use `tcn-opener`).
---

# The Civic Node — Readability Audit (Post-Draft Density Scanner)

## What This Skill Does

Takes a finished TCN article draft and returns a structured report with three parts:
...
The goal is to *help the user revise*, not to auto-produce a fully rewritten draft. Wholesale rewrites by an LLM tend to flatten the voice the writer has been building. The user — or a focused re-run of `tcn-draft` on a specific section — does the final hand. This skill's job is to make the density problems legible and the revision work surgical.
...
- **`tcn-draft`** generated the draft and aimed for these principles at write-time. If this audit reveals that an entire section is the problem, the cleanest fix is often re-running `tcn-draft` on just that section rather than patching here.
- **`tcn-text-humanizer`** covers the *lexical* dimension — AI vocabulary tells, voice-breaking phrases — and rewrites them in Justin's voice. Run `tcn-readability` first (structural) when both apply, then `tcn-text-humanizer` (word-level).
- **`tcn-opener`** covers the opener specifically. If this audit flags only the opener, defer there for the rewrite.
- **`tcn-fact-check`** is a different dimension entirely (source verification). Order: density → AI tells → fact check.

---

## When to Invoke — and When Not To

**Use on:** finished TCN article drafts the user is preparing to publish, or that the user describes as dense, intimidating, or hard to follow. Also use when the user explicitly asks for a readability or accessibility pass even if they haven't named what feels wrong.
...
- Drafts that haven't been through `tcn-draft` yet — running readability on raw conversation transcripts or first-pass brainstorms produces false positives
- Non-TCN prose — the principles are derived from TCN's voice and reader (Marcus). For generic readability work the principles still apply, but the audience calibration in Step 3 won't transfer cleanly
...
**Calibration matters.** A piece on regulatory finance can carry more density than a general-audience explainer; the reader self-selected for the load. Read the draft's apparent audience before grading severity, not after. For Marcus specifically (TCN's archetypal reader), Marcus is sophisticated but busy — he tolerates density that earns its keep and quits on density that doesn't.
...
**This skill flags; it doesn't overwrite.** A fully auto-rewritten draft tends to lose the voice the user has been building through `tcn-draft`. The user's hand — or a focused re-run of `tcn-draft` on a specific section — does the final revision. The report's job is to make the problems legible and the fix specific.

**Density is not a vice in itself.** TCN's reader rewards information density. The failure mode this skill targets is not density per se; it's *uncalibrated* density — passages that ask the reader to hold more than they can comfortably carry. Some pieces legitimately run dense; the audit's job is to find the spots where the density wasn't earned.
...
**Calibrate to audience, not to a universal threshold.** A piece on derivatives pricing for derivatives traders runs denser than a piece on housing for general readers. The thresholds in `references/density-patterns.md` are starting points, not absolutes. Adjust severity based on who the piece is for.
...
1. **`tcn-readability`** (this skill) — structural density audit
2. **`tcn-text-humanizer`** — lexical AI-tell pass in Justin's voice (rewrites in place)
3. **`tcn-fact-check`** — source verification
4. **`tcn-fact-reconcile`** — apply fact-check corrections

This order is not arbitrary: structural cuts can eliminate sentences that `tcn-text-humanizer` would otherwise have rewritten, and rewriting density problems after fact-checking risks reintroducing unsourced claims. Density first, lexical second, sourcing last.

**`tcn-opener`** is independent — if only the opener is the problem, run that instead.
