...
name: blog-opener
description: >
  Drafts or evaluates the opening paragraph(s) of a blog article. Invoke when the user
  ...
  to social media posts, full article drafts, or source/reference operations.
---

# Blog Opener Skill

Resolve the active blog profile per `~/.claude/blog-profiles/_resolution-contract.md` before doing anything else.
Loads: `reader.md`, `voice.md`, and the active preset.

## What This Skill Does

Drafts or evaluates the opening paragraph(s) of a blog article. Operates in two modes:
...
If a vocabulary or substitution decision is needed mid-task, resolve it by referring to the active profile's `voice.md` at runtime, not by relying on a copy embedded in this spec. Any short examples cited here are illustrative only — the profile's `voice.md` is authoritative.
...
**Future-work hook — adjacency-aware calibration.** The profile's `voice.md` may note a gloss-aggressiveness rule that is conservative; a future enhancement would vary gloss aggressiveness by which adjacent cohort each piece targets. NOT IN SCOPE this pass. When implemented, the technique-selection step (specifically which broad hook to reach for) would consume an adjacency signal — pieces targeting a wider cohort lean toward the lighthouse with a more universal broad-end; pieces targeting a denser cohort can carry more domain-specific opening images.

---

## The Lighthouse Model

The blog's author voice (see `voice.md`) opens with an **analogy that narrows from broad to specific**. The lighthouse attracts ships that weren't looking for it.
...
Six opener approaches drawn from the active author voice. The lighthouse is the default — the others are variations or alternatives when the piece calls for something different.

### 1. The Analogy That Narrows (the lighthouse)
**What it is:** Open with something broadly accessible; narrow to the specific thesis.
**Best for:** Concept-explaining pieces, unexpected-connection pieces, pieces where the reader might not know they care about the topic yet.
...
**Best for:** Multi-signal pattern pieces, paradox-driven pieces, early-warning pieces.
**Structure:** `[What the model predicts.] [What actually happened.] [This piece explains why.]`
**Example:** "By every standard model of supply and demand, DDR5 should be trending toward $200 by now. It's at $400."
...
**Best for:** Origin stories, current-event-anchored pieces, news-peg pieces.
...
**Best for:** Early-warning pieces, named-hypocrisy pieces, data-heavy pieces.
...
**Best for:** System-analysis pieces, named-hypocrisy pieces, pieces where the mainstream framing is wrong.
...
**Example:** "Three companies control 90% of the world's DRAM production. Two of them are on the same peninsula. One shipping lane carries their primary process gas. This was always going to happen."
...
**Example:** "Why would a war in the Middle East affect your RAM?"
...
These are opener-form failure modes, kept in this skill because each one has a reason it specifically fails *as an opener* (not just as prose). For the broader banned-phrase catalog (any context, not just openers), see the active profile's `voice.md`. Phrases listed here that also appear in the voice file are not contradicting it — they're flagged here for their opener-specific failure mode; the profile's `voice.md` is authoritative on the phrase ban itself.
...
| Personal anecdote about the writer's day | Too slow to narrow. |
...
Each content structure calls for a different opener approach. The active profile's `templates.md` defines the exact structure library; use these generic technique pairings as defaults, adapted to whichever template names the profile uses.

**Multi-signal convergence pieces** (multiple signals → thesis)
The opener must name the convergence before the body explains each signal. Don't tease — tell. The power is that the reader knows all the forces exist before encountering any one of them; the body then delivers on each.
→ Best technique: Expectation-Subversion, then name the forces.

**System-analysis pieces** (problem → analysis → solutions)
...
**Current-event-anchored pieces** (current event → historical context → future implications)
...
**Concept-explaining pieces** (definition → mechanics → applications)
...
2. **Identify the content structure** → use the template-specific guidance above (adapted to the active profile's `templates.md`) to narrow the technique options.
...
7. **Would the reader persona keep reading?** Apply the patience test (see `reader.md`) specifically to the first 100 words.
...
[Opener text — full paragraph(s), platform-appropriate markdown]
...
| Reader persona keeps reading | ✓ / ✗ | [observation] |
...
The full voice reference is the active profile's `voice.md`. Opener-specific reminders:
