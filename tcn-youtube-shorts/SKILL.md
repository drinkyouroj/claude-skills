...
description: "Step 6 of the Civic Node YouTube production workflow — packages a batch of finished 9:16 vertical clips (already cut and rendered from a recorded dispatch) into per-clip YouTube Shorts metadata: a clip-native title and a compact description (hook + Substack article CTA + #Shorts and dispatch hashtags), one paste-ready file per clip with a per-clip review gate. Batch-first — N vertical clips in, N youtube-shorts-clip-NN.md files out. Slices the dispatch's master .srt transcript per clip so each Short's copy comes from the words actually spoken on screen, and inherits the Substack URL and channel boilerplate from the long-form package rather than regenerating them. Invoke when the user says 'package the shorts', 'write the shorts titles', 'shorts for this dispatch', 'youtube shorts for №NNN', 'shorts metadata for №NNN', 'package the vertical cuts', 'per-clip titles and descriptions for these clips', or has finished cutting vertical clips from a dispatch and wants them packaged for upload (those same vertical clips also feed TikTok/Reels, but v1 writes YouTube Shorts metadata). Does NOT generate the article, narration, slideshow, or the long-form title/description/thumbnail (those are separate skills — and in particular it is NOT the single long-form title or description for the whole dispatch; this skill fires only when the request names clips, shorts, or vertical cuts), does NOT cut or render the clips (that's the user's editor), and does NOT upload to YouTube (the user pastes each file into YouTube Studio)."
---
