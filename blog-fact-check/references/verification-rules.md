...
- Note if the quote is from a paywalled source and attributed via secondary reporting

### Claims sourced to paywalled, JS-rendered, or blocked content

The skill's architecture is **always-live + archive-corroboration**, with a three-step escalation chain in Step 2c: WebFetch → Chrome MCP → manual scrape. Most "inaccessible" sources resolve at one of these tiers; the archive is only the verifier of last resort when all three escalation steps fail.
...
4. **Archive-only verification** — only if all three tiers fail AND the archive has an existing page for the URL. Mark medium confidence and add the URL to the Link Health section.
5. **Secondary source check** — if even the archive has nothing, check whether a secondary source (news outlet) reported the same data with attribution. Note in the report.
...
When the archive is the only signal available, mark the verification at **medium confidence** and add the URL to the Link Health section noting the live fetch failed — readers clicking the link will hit the same paywall, which is its own credibility issue worth surfacing.

**Manual scrape isn't a punt — it's an investment.** When the user provides a scraped file, the fact-checker hands it to the source archive's Ingest flow (see Step 2.5 in SKILL.md). The result: future fact-checks against the same URL skip the manual step entirely because the archive now has the source. Over time, the archive absorbs the publication's worth of paywalled/PDF sources and becomes increasingly self-sufficient.

### When the archive and the live source disagree

This is the failure mode the cross-check architecture is designed to catch. Three things can cause it, and they have different fixes:

1. **The source was edited or corrected after archive ingestion.** News outlets quietly update articles all the time — figures get corrected, paragraphs get added, retractions get appended. The live page is the current truth.
...
3. **The archive ingestion was wrong.** The archive extract captured the wrong figure, misattributed a quote, or summarized the source incorrectly at ingestion time. The live page is what the source actually says.

Distinguishing these matters for the recommendation:

- **Edited/corrected source:** correct the article to match live; archive should be re-ingested
- **Replaced/retracted source:** find a new source for the claim, or remove the claim; flag the archive page for retirement or re-ingestion against the new URL
- **Archive ingestion error:** correct the article only if it was relying on the wrong archive summary; flag the archive page for re-ingestion regardless
