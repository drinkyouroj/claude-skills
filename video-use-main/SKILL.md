...
## Highlights mode (shorts + podcast reels)

When the user asks for "exciting moments," "highlights," "the best bits," "a short from this," or "clip reel," treat it as a specialization of the core process — not a different workflow. The LLM reads `takes_packed.md`, identifies standout moments, and returns an EDL. **Do not write a moment-scoring function.** The anti-pattern on line ~298 applies double here: the LLM's qualitative judgment on what lands is the whole point of this mode.

### What "exciting" means (read the text like a listener)

Do not codify this as a rubric to score against. Read the packed transcript as if you were listening to the recording and ask: *would I stop scrolling, save this clip, or quote it to a friend?* Common shapes that read as exciting from text alone:

- **Audio-event reactions** — `(laughs)`, `(applause)`, `(gasps)` tags mark beats the audience already told you were landing. Extend past them; the reaction IS the beat.
- **Self-contained punchlines** — setup + payoff inside one or two phrases, no outside context required.
- **Emphasis + silence** — a landed point followed by ≥400ms pause. The speaker is letting it breathe. That's a signal.
- **Stakes and specificity** — named people, real numbers, concrete consequences. "We lost the Dallas account" beats "this changed everything."
- **Tonal shifts** — shorter sentences, profanity used sparingly, direct address. The speaker is leaning in.

Rank qualitatively by judgment, not score. Note confidence in the EDL `reason` field.

### Target profiles (confirm one in the strategy phase)

These are worked examples — concrete starting points. Deviate when the material demands it.

**`short-vertical`** (default when the user says "make a short"):
- `1080×1920 @ 30fps`
- **Single clip, 15–45s**, one beat, one payoff. Do not stitch beats — a short that cuts between moments dilutes the single-landing feel the format rewards.
- Cut padding tighter than default: **30–80ms**. Hard Rule 7 still governs the floor.
- Subtitles: `bold-overlay` style (UPPERCASE, 2-word chunks, `MarginV=35`). The existing `SUB_FORCE_STYLE` in `render.py` ships with this.
- Open on the payoff or the immediate setup — never on filler, self-introduction, or "so basically..."
- Grade: aggressive is fine but optional. Pacing is the primary lever.

**`podcast-highlight-reel`** (default when the user says "podcast highlights"):
- Source aspect, typically `1920×1080 @ 30fps`
- **3–5 clips assembled chronologically by source time, 2–4 minutes total.** Chronological order preserves the arc of the conversation; ranking-order scrambles it.
- Cut padding: **80–150ms** — more air, conversational feel
- Speaker handoffs between clips: 400–600ms, per standard cut craft
- Subtitles: `natural-sentence` style, or none. User call.
- Optional lower-third card overlay between clips marking topic transition (animation slot, built per the usual animation flow).

**`podcast-standout-single`** (use if user asks for "the best moment" / "one clip" / "the highlight"):
- Same technical specs as the reel
- **Single clip, 45–90s**, one complete thought including setup context
- Do not tighten the cut beyond what the thought warrants — the moment has to breathe. A 60s clip is better than a 35s clip that doesn't land.

Confirm the profile at step 4 (Propose strategy) before spawning the editor sub-agent.

### Highlights editor sub-agent brief

```
You are selecting highlight moments from a <content-type> recording.
Return an EDL of the most compelling moments from takes_packed.md.

INPUTS:
  - takes_packed.md (phrase-level transcript with audio-event tags)
  - Content context: <2 sentences from the user>
  - Speaker(s): <name, role, delivery style>
  - Profile: <short-vertical | podcast-highlight-reel | podcast-standout-single>
  - Target runtime: <seconds — from profile>
  - Must-include moments: <list or "none">
  - Must-avoid moments: <list from pre-scan pass>

WHAT COUNTS AS A HIGHLIGHT:
  Read the packed transcript as a listener. A highlight is a moment where 
  you would stop scrolling, quote the speaker, or save the clip.
  - Audio-event reactions (laughs, applause, gasps) marking beats
  - Self-contained punchlines (setup + payoff inside 1–2 phrases)
  - Emphasis followed by silence (landed point + ≥400ms pause)
  - Stakes / specificity (named people, real numbers, consequences)
  - Tonal shifts (shorter sentences, profanity, direct address)

  Do NOT score. Rank by judgment of what a listener would save or forward.

RULES:
  - Start/end times on word boundaries from the transcript.
  - Pad cut boundaries per profile:
      short-vertical:           30–80ms
      podcast-highlight-reel:   80–150ms
      podcast-standout-single:  80–150ms
  - Prefer silences ≥400ms as cut targets.
  - Preserve laugh/reaction tails — the reaction IS the beat.
  - Open on the payoff or immediate setup; never on filler.
  - For reels: chronological by source time, not by ranking.
  - For standout-single: include enough setup that the moment lands 
    without outside context.
  - For short-vertical: ONE beat. Do not stitch multiple moments.
  - Report total runtime; if over target, drop the weakest beat, 
    do not uniformly trim tails (trimming tails kills landings).

OUTPUT (JSON array, no prose):
  [{"source": "C0103", "start": 12.40, "end": 38.22, "beat": "PUNCHLINE",
    "quote": "...", "reason": "Laugh at 34.8, pause to 37.4. Self-contained."}]

Return the EDL and a one-line runtime check.
```

### Fits into the existing process

Steps 1–3 (Inventory, Pre-scan, Converse) unchanged. At step 4 (Propose strategy), specify the profile and wait for confirmation. At step 5 (Execute), spawn the highlights brief above. Steps 6–8 (Preview, Self-eval, Iterate) unchanged, with one addition to self-eval:

**Stand-alone test** — play each selected clip in isolation. If it needs the surrounding 30 seconds to make sense, widen the cut or drop the clip. Highlights that require unpresented context are not highlights. This is the single most common failure mode of LLM-selected reels.
