---
name: tcn-outline-more
description: >
  Step 1b of the Civic Node Substack article workflow: expands a tcn-outline bullet-point outline
  ...
  tcn-outline output and wants to go deeper before drafting themselves. Does NOT produce prose.
  Does NOT replace tcn-draft. The user still writes every sentence.
---

# The Civic Node — Detailed Outline Generator (Step 1b)

## What This Skill Does

Takes a `tcn-outline` output and expands it into a paragraph-level detailed outline — all facts
...
Load `references/article-templates.md` for full template section detail.
Load `references/marcus-long-form.md` for the long-form reader persona.
Load `references/voice-rules.md` for the draft-form rhetorical principles this skill anticipates (including "Paragraph Rhythm and Grounding Cadence" — the shared source of truth for accessibility used by `tcn-draft` and `tcn-readability`). Banned vocabulary, negative parallelisms, vocabulary cliff, and closing-line abstraction live in the canonical voice file — see next section.
...
- **`tcn-draft`** (Step 3) writes the article from this outline, baking accessibility principles in at generation time (paragraph rhythm, grounding cadence, statistic framing, deliberate reader address — see its "Drafting for Accessibility" section).
- **`tcn-readability`** runs after the draft as a structural audit for density and accessibility, against the same five principles.
...
Review the incoming `tcn-outline` and confirm:

1. **Template selection** — which of the four templates was chosen, and why
2. **Viral trigger** — which of the five triggers the outline targets
3. **Section structure** — how many sections, their names, word count targets
4. **Source availability** — which facts already have wiki sources vs. need flagging
...
Grounding plan: [Where in this paragraph does the abstraction touch ground? Name the move — named entity, specific number, concrete scene, or one-word punch. If the paragraph is already concrete throughout, write "concrete throughout — no extra grounding needed." This field exists so `tcn-readability` finds the grounding already in place rather than flagging the paragraph as ungrounded. See voice-rules.md "Paragraph Rhythm and Grounding Cadence."]
...
- Pattern Report items (200-300 words each): 2-3 paragraphs
...
Justin can decide whether it works and, if so, write it himself.]
Example shape: "[rough natural-language description of the arc — one sentence, not polished prose]"
Why it might work: [one sentence on what it would achieve for Marcus — hook, credibility,
...
Model sentence rhythm: [Reference one of the benchmark sentences from voice-rules.md that has
the right rhythm for this moment]
```
...
Marcus 30-second test: [Would Marcus pass this opener into the full piece? What's the hook that
...
Before handing the detailed outline off to `tcn-draft`, verify it has set the prose up to succeed on the five accessibility principles (audited later by `tcn-readability`):
...
Verdict: [ready for tcn-draft / needs accessibility planning fix before drafting]
```

The point of this check is not to write the prose; it is to make sure the *raw material* is set up so that when `tcn-draft` runs, it has what it needs to draft accessibly on the first pass. Fixing accessibility at outline time is cheaper than fixing it at audit time.

---

## The Marcus Pre-Assessment

Run the five Marcus tests against the detailed outline before presenting:
...
2. Patience test — Would Marcus still be reading at paragraph 3?
   [assessment — name the paragraph most likely to lose him and why]
...
**Not a replacement for the user's voice.** Rhetorical suggestions describe a move, not a voice.
The analogy the AI describes will be written by Justin in Justin's register. The suggestion is
...
"discuss Samsung's Q1 results" are not detailed enough — it's "Samsung posted Q1 2026 operating
profit of 57.2 trillion won ($37.92 billion) — [source]."
...
- `references/marcus-long-form.md` — Marcus persona for long-form
- `references/voice-rules.md` — Voice and style reference: sentence rhythm, **paragraph rhythm and grounding cadence** (shared source of truth for accessibility), rhetorical principles, humor, the AI hit list

## Companion Skills

- **`tcn-outline`** (Step 1) — Bullet-point outline that feeds into this skill.
- **`tcn-headline`** (Step 2) — Headline and subheadline generation from the outline.
- **`tcn-draft`** (Step 3) — Writes the prose from this detailed outline. See its "Drafting for Accessibility" section, which this outline's Grounding plan and Statistic framing fields are designed to set up.
- **`tcn-readability`** — Post-draft structural audit. The Accessibility Pre-Check above directly determines how light a touch `tcn-readability` will need on the back end.
- **`tcn-text-humanizer`** — Post-draft lexical pass for AI tells, calibrated to Justin's voice (rewrites in place). Independent of accessibility.
- **`tcn-opener`** — Standalone opener fix-pass if needed after drafting.
- **`tcn-fact-check`** / **`tcn-fact-reconcile`** — Source verification on the finished draft.
