...
4. If `edit-diff.json` exists with `status: captured` and `pair_count > 0`, present Proposal Mode as an optional voice-learning handoff:

```text
python3 ~/.claude/skills/blog-voice-patch/scripts/write_rule_patch_proposals.py --workflow-dir <workflow-dir> --edit-diff edit-diff.json --proposal-input <proposal-input.json>
```
