...
name: tcn-draft
description: >
  Step 3 of the Civic Node Substack article workflow: full section-by-section draft from an approved
  outline and headline. Invoke this skill when the user has approved both an outline (tcn-outline)
  and a headline (tcn-headline) and wants to write the article — including phrases like "write the
  ...
  tcn-post), outlines (tcn-outline), headlines (tcn-headline), fiction, or wiki operations.
---

# The Civic Node — Article Draft Generator (Step 3 of 3)
...
`tcn-draft` is one of two skills that share responsibility for prose accessibility in the TCN workflow:

- **`tcn-draft` (this skill)** generates the draft and bakes accessibility principles in from the start — paragraph rhythm, grounding cadence, statistic framing, deliberate reader address. These are generation-time choices that are easier to make once than to fix later.
- **`tcn-readability` (audit pass)** runs after the draft and audits the same accessibility principles structurally — flagging paragraphs that ran too long, abstract claims that never touched ground, statistics stated naked, flat rhythm, and sentence-level comprehension drag.

The two are intentionally aimed at the same target from different angles. `tcn-draft` aims for the principle during generation; `tcn-readability` enforces it on the back end. The principles are stated once in detail — here under "Drafting for Accessibility" and in `references/voice-rules.md` under "Paragraph Rhythm and Grounding Cadence." `tcn-readability` audits against the same source of truth rather than restating it. If a principle changes, update it here and `tcn-readability` will pick up the new bar on its next audit.

`tcn-text-humanizer` is a separate, lexical-level pass (AI tells, voice-breaking vocabulary) calibrated to Justin's voice — it rewrites in place rather than producing a report. When both apply, run `tcn-readability` first (structural fixes) and `tcn-text-humanizer` second (word-level cleanup).
...
Write conversationally — like Justin emailing a specific smart person he respects. Not a brand, not a bot, not a thought leader performing thought leadership. The writing is reaching out. It should feel like contact.
...
4. **Cite sources inline** — footnote links for data claims, hyperlinks for direct references. More citations is always better than fewer. The citations are for Marcus's trust
5. **Cross-link to wiki pages** where relevant (if publishing from the vault)
...
- The analogy should be broadly accessible — something Marcus and a non-specialist both recognize
...
Follow the template structure from the outline. Each section has its own character:

### Section Subheadlines

Every section heading gets a subheadline — a short phrase after the template section name that previews the section's specific argument. Format: `## The Source Code: How a Non-Binding Letter Became a Binding Market`

The subheadlines serve two purposes:
1. **Skimmability** — someone scanning only the headers should get the argument's shape
2. **Patience renewal** — each subheadline gives Marcus a reason to keep reading into the next section
...
**Analytical sections** (The Pattern, The Source Code, The Mechanics):
...
**Action sections** (The Protocol, The Upgrade, The Applications):
...
- Connect to the core domain: who controls power, and do individuals have recourse?

**Personal reflection sections** (Personal Code, My Debug, The Human Element):
...
Every factual claim — data points, quotes, dates, dollar figures, percentages — gets an inline hyperlink. This is non-negotiable. Marcus follows source links. One unsourced claim on something he can check destroys credibility across the entire piece.

**Do not use footnotes.** Sources are embedded as inline links on a few key words within the sentence that states the claim. When the draft is pasted into Substack, the links carry over directly — no manual footnote conversion needed.
...
- Every link must point to a real, verifiable URL from the wiki's source pages (check `source_url` in frontmatter) or from verified external reporting. Never use generic domain links.
...
1. **Callback to the opener** — restate the opening analogy in the light of what the piece established. The same image, transformed by the argument
2. **Leave Marcus pondering** — the close adds one more thing. Not a conclusion he already drew from the setup; a reframe, an implication, or a question worth investigating
...
The **tonal drop + micro-reframe** from the social process applies here too. The close can shift register (from analytical to personal, from precise to casual) but must carry a new idea inside the shift. Tone change without payload is sarcasm without a point.

---

## Drafting for Accessibility

These five principles shape readability from the moment a sentence is written. They are the generation-time half of the accessibility work — `tcn-readability` (post-draft audit) checks the same five principles structurally on the finished draft. See `references/voice-rules.md` under "Paragraph Rhythm and Grounding Cadence" for the full explanation; this section is the working summary.

The reason these matter: TCN's reader (Marcus) tolerates and rewards density, but only when the density is earned. Uncalibrated density — paragraphs that go on, abstractions that don't touch ground, numbers that don't argue anything — is the difference between a piece Marcus saves and a piece he closes at paragraph three even though every claim is correct.
...
- "I" still appears only in personal reflection sections; "we" remains off-limits (it presumes shared identity with the reader, which TCN does not assert)
...
## The Five Long-Form Marcus Tests

Run these against the complete draft before presenting:

1. **Signal test** — Does this deliver at least one thing Marcus couldn't have found easily himself?

2. **Patience test** — Would Marcus still be reading at paragraph 3? Is the argument advancing, not restating?
...
4. **Save test** — Would Marcus save this to reference later, or forward it to one specific person he respects?

5. **Accumulation test** — Does this contribute to the "slightly ahead of him" pattern? Does it raise or maintain the signal floor?
...
Present the draft as clean Substack-ready markdown:
...
[Opener — the analogy that narrows]

[Thesis — by paragraph 2]

---

## [Template Section Name]: [Section Subheadline]
...
- Template: [which]
- Trigger: [which]
- Marcus tests: [pass/fail summary — flag any concerns]
...
## What Makes a TCN Draft Different from Generic Newsletter Prose
...
The difference is conviction backed by evidence, delivered with personality. That's what Marcus stays for.

---

## Reference Files

- `references/voice-rules.md` — Full voice reference: punctuation philosophy, sentence rhythm, **paragraph rhythm and grounding cadence**, words to love/avoid, model sentences, the AI hit list, humor principles
- `references/article-templates.md` — Full template detail: section structures, word counts, tone notes per section, trigger-template pairings

## Companion Skills

- **`tcn-readability`** — Post-draft accessibility audit. Runs the same five principles in "Drafting for Accessibility" above as a structural check on the finished draft. Use after `tcn-draft` and before publishing whenever the writing feels dense or hard to follow.
- **`tcn-text-humanizer`** — Post-draft lexical pass for AI tells, calibrated to Justin's voice (active rewriter, not a report). Run after `tcn-readability` when both apply (structural fixes precede word-level cleanup).
- **`tcn-opener`** — Standalone opener fix-pass when the opening paragraph is the weak point.
- **`tcn-fact-check`** / **`tcn-fact-reconcile`** — Source verification and correction application. Independent dimension from voice and readability.
